...
class Situation:
    def __init__(self, name, intro, question, option_1, option_2, inventory):
        self.name = name
        self.intro = intro
    ...
    def choice(self):
        ...
        if user_input == "armory room":
            if user_input in armory_room.inventory:
                items.append(user_input)
                armory_room.remove(user_input)
                return items
...
ending_2 = Situation("OUTSIDE", "YOU WIN! You go outside of the closet and you look out the window in the living room.. You see a tree with a hole that seems like there is a light on the other side. You quickly run outside, go through the hole, and.. Youre home!!", "", "", "", [items])

death_8 = Situation("HOME", "GAME OVER! You lost track of time and sat down to close your eyes out of exhaustion and dehydration and soon die.", "", "", "", [items])

stay_still2 = Situation("CLOSET", "Good choice! The Demagorgan was still nearby so it is best to wait!", "How long do you want to wait for? (5 mins or 15 mins): ", {"5 mins":[ending_2]}, {"15 mins": [death_8]}, [items])

death_6 = Situation("LIVING ROOM", "You were in the direct eyesight from the Demagorgan and once he saw you he killed you.. GAME OVER.", "", "", "", [items])

death_4 = Situation("MIKES HOUSE", "GAME OVER. You go to your long time friends Mike's home. This place was always the hang out house for your friend group so the familiarity makes you happy to get home but hat feeligng is soon erased by exhaustion. You soon fall asleep where the Demagorgan finds you and kills you.", "", "", "", [items])

now_outside = Situation("OUTSIDE","There was nothing else that was useful ")

armory_room = Situation("ARMORY ROOM", "You enter the armory room and see three possible weapons. The backpack to the left of the weapons can only fit one weapon. Your options are a knife, a flamethrower, or a gun.", "Which weapon do you choose? (flamethrower or gun): ", "flamethrower", "gun", ["flamethrower", "gun"])

police_station = Situation("POLICE STATION", "You enter the police and it is empty but full of the filth from the Upside Down. To your left there's an armory room.. Do you want to enter the armory room?", {"yes": [armory_room]}, {"no": [death_8]}, [items])

look_around = Situation("OUTSIDE", "It is stormy outside and you miss the fake safety of your 'home'. You look around and remember that Mike's house is to the left while the police station is to the right", "Where do you want to go? (mikes house or police station)", {"mikes house": [death_4]}, {"police station": [police_station]}, [items])
